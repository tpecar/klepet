...
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      ...
      /* poiscemo vzdevek izvora */
      var vzdevekIzvor = vzdevkiGledeNaSocket[socket.id];
      /* preverimo se, ce je naslovnik kar posiljatelj ter mu to onemogocimo */
      if (socket.id == socketIdNaslovnika) {
       ...
       io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', { dregljaj: true, izvor: vzdevekIzvor });
      } else {
       io.sockets.sockets[socket.id].emit('sporocilo', {
